...
export const SubjectImpl = ({ onSuccessCreate }) => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            await axios.post("https://student-be-production.up.railway.app/objectives/new", dataToSend);
            setShowModal(false);
    ...
    useEffect(() => {
        axios.get(`https://student-be-production.up.railway.app/subjects/subjectId=${selectedSubjectId}`)
            .then(response => response.data && setSubject(response.data))
            .catch(error => console.error("Error fetching subject:", error));
    }, [selectedSubjectId]);

    useEffect(() => {
        axios.get(`https://student-be-production.up.railway.app/subjects/${selectedSubjectId}/skills`)
            .then(response => {
    ...
    useEffect(() => {
        if (skillIndicator.length > 0) {
            Promise.all(skillIndicator.map(skill => axios.get(`https://student-be-production.up.railway.app/indicators/skillId=${skill.id}`)))
